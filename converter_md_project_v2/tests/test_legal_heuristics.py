...
class TestAcaoH1Pattern:
    """Bug 2: AÇÃO REVISIONAL and variants must become H1."""

    def test_acao_revisional_uppercase(self):
        text = "AÇÃO REVISIONAL DE CONTRATOS DE CRÉDITO RURAL"
        result = apply_legal_heuristics(text, mode="forense")
        assert result.strip().startswith("# ")

    def test_acao_indenizacao_uppercase(self):
        text = "AÇÃO DE INDENIZAÇÃO POR DANOS MORAIS"
        result = apply_legal_heuristics(text, mode="forense")
        assert result.strip().startswith("# ")

    def test_acao_declaratoria(self):
        text = "AÇÃO DECLARATÓRIA DE NULIDADE"
        result = apply_legal_heuristics(text, mode="forense")
        assert result.strip().startswith("# ")

    def test_acao_mixed_case_not_h1(self):
        """Ação em mixed case (ementa) NÃO deve virar H1."""
        text = "Ação de indenização por danos morais."
        result = apply_legal_heuristics(text, mode="forense")
        assert not result.strip().startswith("# ")


class TestFormatPedidoItems:
    """Bug 5: Itens de enumeração em seção de pedidos devem virar lista markdown."""

    def test_pedido_items_formatted(self):
        text = (
            "## DOS PEDIDOS\n\n"
            "a) o deferimento da tutela provisória\n"
            "b) a revisão das cláusulas contratuais\n"
            "c) a condenação do réu\n"
        )
        result = apply_legal_heuristics(text, mode="forense")
        assert "- a) o deferimento" in result
        assert "- b) a revisão" in result
        assert "- c) a condenação" in result

    def test_pedido_items_not_outside_section(self):
        """Itens fora da seção de pedidos NÃO devem virar lista."""
        text = (
            "## DOS FATOS\n\n"
            "a) o autor celebrou contrato\n"
            "b) o réu descumpriu\n"
        )
        result = apply_legal_heuristics(text, mode="forense")
        assert "- a)" not in result

    def test_long_paragraph_not_formatted(self):
        """Parágrafos longos na seção de pedidos NÃO viram lista."""
        long_text = "a) " + "x" * 250
        text = f"## DOS PEDIDOS\n\n{long_text}\n"
        result = apply_legal_heuristics(text, mode="forense")
        assert "- a)" not in result
