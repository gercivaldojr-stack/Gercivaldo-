...
def apply_legal_heuristics(
    ...
    # F6: Formatar itens de pedidos como lista markdown
    if mode in ("forense", "google"):
        structured = format_pedido_items(structured)
...
def _apply_forense(line: str) -> str:
    ...
    for pattern in FORENSE_H1_PATTERNS:
        if re.match(pattern, upper_line, re.IGNORECASE):
            # AÇÃO patterns: only match if original line is actually uppercase
            # to avoid promoting mixed-case sentences like "Ação de indenização..."
            if pattern.startswith(r"^(AÇÃO"):
                words = line.split()
                if len(words) >= 2 and not words[1].isupper():
                    continue
            return f"# {line}"
...
# Padrões de heading que indicam seção de pedidos
_PEDIDO_HEADING_RE = re.compile(
    r"^#{1,3}\s+.*(?:PEDIDOS?|REQUERIMENTO|REQUER)\b", re.IGNORECASE
)

# Padrão de item de enumeração que pode virar item de lista
_PEDIDO_ITEM_RE = re.compile(
    r"^([a-z]\)|[a-z]\.|[ivxlc]+\)|[IVXLC]+\)|\d+\)|\d+\.)\s+(.+)"
)


def format_pedido_items(text: str) -> str:
    """Formata itens de enumeração dentro de seções de pedidos como lista markdown.

    Após detectar um heading com PEDIDO/REQUERIMENTO, trata linhas curtas
    (< 200 chars) que começam com padrão de enumeração como itens de lista
    (prefixando com '- '). Parágrafos longos de argumentação não são afetados.
    """
    lines = text.split("\n")
    result = []
    in_pedido_section = False

    for line in lines:
        stripped = line.strip()

        # Detectar entrada em seção de pedidos
        if _PEDIDO_HEADING_RE.match(stripped):
            in_pedido_section = True
            result.append(line)
            continue

        # Sair da seção de pedidos ao encontrar outro heading
        if stripped.startswith("#") and not _PEDIDO_HEADING_RE.match(stripped):
            in_pedido_section = False
            result.append(line)
            continue

        # Dentro da seção de pedidos, formatar itens de enumeração
        if in_pedido_section and stripped and len(stripped) < 200:
            m = _PEDIDO_ITEM_RE.match(stripped)
            if m:
